...
func TestSm4Cipher_Encrypt(t *testing.T) {
	for _, data := range testData {
		...
		encrypter.CryptBlocks(result, util.PKCS7Padding(data.in, BlockSize))
		//encrypter.CryptBlocks(result, pkcs7Padding_sm4(data.in, BlockSize))
		...
		plain,err = util.PKCS7UnPadding(plain,BlockSize)
		if !bytes.Equal(plain, data.in) {
			t.Error("decrypt result not equal expected",err)
			return
		}
	}
}
