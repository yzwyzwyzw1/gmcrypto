...
func TestCreateSelfSignedCertificate(t *testing.T) {
	...
	//sm2Privatekey := sm2PrivateKey{sm2Priv}
	sm2Privatekey := sm2Priv
	//sm2Publickey := sm2PublicKey{&sm2Priv.PublicKey}
    sm2Publickey := &sm2Priv.PublicKey
	tests := []struct {
		name      string
		pub, priv interface{}
		checkSig  bool
		...
		{"SM2/SM2",sm2Publickey,sm2Privatekey,false,SM2WithSM3},
	}
...
func TestCreateCertificateSM2withSM3(t *testing.T) {
	random := rand.Reader

	sm2Privkey,err := sm2.GenerateKey(random)

	//sm2Priv := &sm2PrivateKey{sm2Privkey}
	sm2Priv :=sm2Privkey
	if err != nil {
		t.Fatalf("Failed to generate SM2 key: %s", err)
	}
	...
	template := CertificateRequest{
		...
		SignatureAlgorithm: sm2Content.sigAlgo,
		DNSNames:           []string{"test.example.com"},
		EmailAddresses:     []string{"gopher@golang.org"},
		IPAddresses:        []net.IP{net.IPv4(127, 0, 0, 1).To4(), net.ParseIP("2001:4860:0:2001::68")},
	}

	derBytes, err := CreateCertificateRequest(random, &template, sm2Content.priv)
	if err != nil {
		t.Errorf("%s: failed to create certificate request: %s", sm2Content.name, err)
	}
	...
	fmt.Println("name",out.PublicKey)

	err = out.CheckSignature()

	if err != nil {
		t.Errorf("%s: failed to check certificate request signature: %s", sm2Content.name, err)

	}

	if out.Subject.CommonName != template.Subject.CommonName {
		t.Errorf("%s: output subject common name and template subject common name don't match", sm2Content.name)
	} else if len(out.Subject.Organization) != len(template.Subject.Organization) {
		t.Errorf("%s: output subject organisation and template subject organisation don't match", sm2Content.name)
	} else if len(out.DNSNames) != len(template.DNSNames) {
		t.Errorf("%s: output DNS names and template DNS names don't match", sm2Content.name)
	} else if len(out.EmailAddresses) != len(template.EmailAddresses) {
		t.Errorf("%s: output email addresses and template email addresses don't match", sm2Content.name)
	} else if len(out.IPAddresses) != len(template.IPAddresses) {
		t.Errorf("%s: output IP addresses and template IP addresses names don't match", sm2Content.name)
	}



	fmt.Println(out.Subject.CommonName)
