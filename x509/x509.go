...
func marshalPublicKey(pub interface{}) (publicKeyBytes []byte, publicKeyAlgorithm pkix.AlgorithmIdentifier, err error) {
	...
	case *sm2.PublicKey:
		...
		publicKeyAlgorithm.Algorithm = oidPublicKeyECDSA //借用ECDSA的oidPublicKey
...
var signatureAlgorithmDetails = []struct {
	...
	//GM
	{SM2WithSM3, "SM2-SM3", oidSignatureSM2WithSM3, SM2, crypto.SM3},
...
var (
	...
	//oidPublicKeySM2 = asn1.ObjectIdentifier{1, 2, 840, 10045, 2, 1} //这个地方存在问题  ,使用oidPublicKeyECDSA的证书公钥标识
...
//func namedCurveFromOIDSM(oid asn1.ObjectIdentifier) sm2.P256V1Curve {
//
//	if oid.Equal(oidNamedCurveP256SM2) {
//		sm2P256V1Curve :=sm2.GetSm2P256V1()
//		sm2P256V1Curve.Name = "SM2"
//		return sm2P256V1Curve
//	}else {
//		sm2P256V1Curve :=sm2.GetSm2P256V1()
//		sm2P256V1Curve.Name = "NOSM2"
//		return
//	}
//
//
...
// CheckSignature verifies that signature is a valid signature over signed from
// a crypto.PublicKey.
func checkSignatureSM(algo SignatureAlgorithm, signed, signature []byte, publicKey sm2.PublicKey) (err error) {
	...
	if pubKeyAlgo == SM2 {
       ...
       if !pub.Verify(digest,signature) {
...
func parsePublicKey(algo PublicKeyAlgorithm, keyData *publicKeyInfo) (interface{}, error) {
	...
	case DSA:
		...
		return pub, nil
	case ECDSA: //我的SM2使用了ECDSA的iod身份表示
		paramsData := keyData.Algorithm.Parameters.FullBytes
		...
		namedCurveSm2 := namedCurveFromOID(*namedCurveOID)
		//fmt.Println("namedCurveSm2name",namedCurveSm2.Params().Name)
		//fmt.Println("namedCurveSm2.Params().Name",namedCurveSm2.Params().Name)
		if namedCurveSm2.Params().Name == "SM2-P-256" {

		//	fmt.Println("yzw")

			x, y := elliptic.Unmarshal(namedCurveSm2, asn1Data)
			if x == nil {
				return nil, errors.New("x509: failed to unmarshal P256V1Curve point")
			}
			pub :=sm2.PublicKey{
				X:     x,
				Y:     y,
...
func CreateCertificateRequest(rand io.Reader, template *CertificateRequest, priv interface{}) (csr []byte, err error) {
	...
	var digest []byte
	//在使用SM3算法是，执行crypto.SM3.New()报不可访问的错误，没办法，以下先采用分别处理的办法
	//if hashFunc != 20 {
	//
	//	h := hashFunc.New()
...
func (c *CertificateRequest) CheckSignature() error {
   ...
   case sm2.PublicKey:
	  return checkSignatureSM(c.SignatureAlgorithm, c.RawTBSCertificateRequest, c.Signature, c.PublicKey.(sm2.PublicKey))
   }
...
func ParseSM2PublicKey(der []byte) (*sm2.PublicKey, error) {
	...
	fmt.Println("123456")
	if _, err := asn1.Unmarshal(der, &pubkey); err != nil {   //次数反序列化失败
		return nil, err
	}
	fmt.Println("22222")
...
func SM2PrivateKeytoMem(key *sm2.PrivateKey, pwd []byte) ([]byte, error) {
	var block *pem.Block
